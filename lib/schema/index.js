...
module.exports = function (db) {
  ...
  const PokemonUpgradeCost = new graphql.GraphQLObjectType({
    name: 'PokemonUpgradeCost',
    description: 'Constants used in calculating power up costs (eg: stardust, candy).',
    fields: {
      level: {
        type: graphql.GraphQLFloat,
        description: 'This is the Pokemon level'
      ...
      candy: {
        type: graphql.GraphQLInt,
        description: 'This is the number of candy required to power up to this level.'
      },
      stardust: {
        type: graphql.GraphQLInt,
        description: 'This is the number of stardust required to power up to this level.'
      }
    }
  })
  ...
  const query = new graphql.GraphQLObjectType({
    ...
    fields: {
      ...
      cpMultipliers: {
        ...
        resolve (root) {
          const cpMultipliers = settings.by('id', 'CP_MULTIPLIERS')
          return Object.keys(cpMultipliers.levels).map(parseFloat).sort((a, b) => a - b).map(level => {
            return { level, multiplier: cpMultipliers.levels[level] }
          })
        }
      },
      upgradeCosts: {
        type: new graphql.GraphQLList(PokemonUpgradeCost),
        description: 'Used to query for power up costs.',
        resolve (root) {
          const costs = settings.by('id', 'UPGRADE_COSTS')
          return Object.keys(costs.levels).map(parseFloat).sort((a, b) => a - b).map(level => {
            return Object.assign({ level }, costs.levels[level])
          })
        }
      }
    }
